...
**注意：写本工具目的不是一键脱壳，而是能更轻松地观察VMP的虚拟化原理，使用者通过本工具了解了虚拟机的基础原理后可以通过自行编写各种脚本尝试目前各种主流的去虚拟化方式。**
...
2. 一定要把RIP停到进入虚拟机前的push中，如下图所示，注意需要通过断点断到之前运行的目录中然后通过F7执行到虚拟机入口（push xxxxx).
3. 运行 vmp_engine.exe，点击"解析"
4. 点击"Lua"按钮执行分析脚本（如 `demo_llvm.lua`）
