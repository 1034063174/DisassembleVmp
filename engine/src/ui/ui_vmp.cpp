...
static const char* const lua_doc[] = {
    ...
    "@set_clipboard(text)\tvoid\t将文本写入系统剪贴板",
    "",
    "^LLVM 函数 (需要 LLVM-C.dll)",
    "@llvm_available()\tboolean\t检查 LLVM-C.dll 是否已加载",
    "@llvm_optimize_ir(ir, level)\tstring,string\t优化 IR, 返回 (优化后IR, err)",
    "@llvm_emit_asm(ir, level)\tstring,string\t优化+编译为 x86-64 汇编 (Intel), 返回 (asm, err)",
    "//level: 0=O0, 1=O1, 2=O2(默认), 3=O3",
    "//成功时 err=nil, 失败时第一个返回值=nil",
    "",
    "#rows[]  每条指令 (1-based)",
    "@.addr\tinteger\t地址",
    ...
    "@.type\tstring\thandler 类型 (见下表)",
    "@.detail\tstring\t细节 (寄存器名/立即数值/偏移)",
    "@.seg_idx\tinteger\t段索引",
    ...
    "",
    "$  handler 类型 (split_handlers.lua 识别):",
    "//  vPop         -- 从 vmStack 弹出值到寄存器槽位",
    "//  vPushReg     -- 从寄存器槽位读值压入 vmStack",
    "//  vPushImm64   -- 从 vmCode 读 8 字节常量压入 vmStack",
    "//  vPushImm32   -- 从 vmCode 读 4 字节常量压入 vmStack",
    "//  vPushImm16   -- 从 vmCode 读 2 字节常量压入 vmStack (movzx)",
    "//  vPushVSP     -- 压入 vmStack 指针自身",
    "//  vAdd/vSub    -- 算术运算",
    "//  vAnd/vOr/vXor -- 逻辑运算",
    "//  vNand        -- NOT+NOT+OR = ~(a & b)",
    "//  vNor         -- NOT+NOT+AND = ~(a | b)",
    "//  vNot/vNeg    -- 一元运算",
    "//  vShr/vShl/vSar -- 移位运算",
    "//  vStore64/32/8 -- 写内存",
    "//  vLoad64/32/8  -- 读内存",
    ...
    ">end",
    "",
    "#Demo 8: LLVM 一键还原 (执行 demo_llvm.lua)",
    ">dofile(\"scripts\\\\demo_llvm.lua\")",
    "//自动执行: annotate -> split_handlers -> lift IR -> LLVM O2 -> 输出汇编",
    "",
    "#Demo 9: LLVM 手动调用",
    ">if not llvm_available() then log(\"[error] LLVM-C.dll 未加载\"); return end",
    ">",
    ">local ir = [[",
    ">define void @test() {",
    ">entry:",
    ">  %0 = call i64 asm sideeffect \"\", \"={rcx}\"()",
    ">  %1 = add i64 %0, 1",
    ">  %2 = sub i64 %1, 1",
    ">  call void asm sideeffect \"\", \"{rcx}\"(i64 %2)",
    ">  ret void",
    ">}",
    ">]]",
    ">",
    ">local asm_text, err = llvm_emit_asm(ir, 2)",
